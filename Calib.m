...
for fb = 1:size(FB,1)
    ...
    ind_tmp = randperm(length(mrk.pos));
    div = floor(length(mrk.pos)/10);
    
    for i = 1:10
        batch(i,:) = ind_tmp(1+(i-1)*div:i*div);
    end
...
%         [U, phsi] = eig(S1,S1+S2+S0);
    ...
    P = P_batch{1} + P_batch{2} + P_batch{3} + P_batch{4} + P_batch{5} + P_batch{6} + P_batch{7} + P_batch{8} + P_batch{9} + P_batch{10};
    P = P/10;
